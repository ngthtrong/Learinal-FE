...
function AdminDashboardPage() {
  ...
  async function loadData() {
    try {
      ...
      try {
        ...
        const subsActs = (subsRes?.items || []).map((s) => {
          ...
          return {
            ...
            dateIso: date || null,
            label: t("admin.dashboardPage.userPurchasedPlan", { userName: s.userName || t("admin.dashboardPage.user"), planName: s.planName || "—" }),
            status: s.status,
          };
        });
        const paidActs = (paidRes?.items || []).map((c) => {
          ...
          return {
            ...
            dateMs: date ? new Date(date).getTime() : 0,
            dateIso: date || null,
            label: t("admin.dashboardPage.commissionPaidTo", { expertName: c.expertName || t("admin.dashboardPage.expert"), amount: (c.commissionAmount || 0).toLocaleString(language === "vi" ? "vi-VN" : "en-US", { style: "currency", currency: "VND" }) }),
          };
        });
      ...
      console.error(e);
      setError(e?.response?.data?.message || t("admin.dashboardPage.cannotLoadStats"));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const cards = [
    {
      key: "users",
      label: t("admin.dashboardPage.users"),
      value: stats?.users?.total ?? 0,
      icon: UsersIcon,
      color: "from-indigo-500 to-indigo-600",
      footer: stats ? t("admin.dashboardPage.activeCount", { count: stats.users?.active ?? 0 }) : undefined,
    },
    {
      key: "plans",
      label: t("admin.dashboardPage.upgradePlans"),
      value: plansSummary.total,
      icon: SubscriptionsIcon,
      color: "from-emerald-500 to-emerald-600",
      footer: t("admin.dashboardPage.plansSummary", { active: plansSummary.active, inactive: plansSummary.inactive }),
    },
    {
      key: "user_subs",
      label: t("admin.dashboardPage.userPackages"),
      value: userSubsTotal,
    ...
    {
      key: "commission",
      label: t("admin.dashboardPage.unpaidCommission"),
      value: loading
        ? "—"
        : unpaidCommissionTotal.toLocaleString(language === "vi" ? "vi-VN" : "en-US", { style: "currency", currency: "VND" }),
      icon: CoinsIcon,
      color: "from-amber-500 to-amber-600",
      footer: t("admin.dashboardPage.recordCount", { count: loading ? "—" : unpaidCommissionCount }),
    },
  ];
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-slate-900 py-6 sm:py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100">{t("admin.dashboardPage.title")}</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-xs sm:text-sm">{t("admin.dashboardPage.subtitle")}</p>
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            >
            {t("admin.dashboardPage.exportReport")}
            </button>
          </div>
        </div>
        ...
        {!loading && currentMonthData && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-5 border border-gray-100 dark:border-slate-700">
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t("admin.dashboardPage.subscriptionRevenueMonth", { month: currentMonth })}</div>
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                {formatCurrency(currentMonthData.subscriptionRevenue || 0)}
              </div>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-5 border border-gray-100 dark:border-slate-700">
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t("admin.dashboardPage.commissionExpenseMonth", { month: currentMonth })}</div>
              <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                {formatCurrency(currentMonthData.commissionsPaid || 0)}
              </div>
            </div>
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-5 border border-gray-100 dark:border-slate-700">
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t("admin.dashboardPage.netProfitMonth", { month: currentMonth })}</div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-8 mb-8">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">{t("admin.dashboardPage.recentActivities")}</h2>
          {activitiesLoading && (
            <div className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">{t("admin.dashboardPage.loadingActivities")}</div>
          )}
          {!activitiesLoading && activities.length === 0 && (
            <div className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">
              {t("admin.dashboardPage.noRecentActivities")}
            </div>
          )}
              ...
              <li key={`${act.type}_${act.id}`} className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  {act.type === "subscription_purchase" && (
                    <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">{t("admin.dashboardPage.status")}: {act.status}</div>
                  )}
                  {act.type === "commission_paid" && (
                    <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">{t("admin.dashboardPage.commissionPaid")}</div>
                  )}
                </div>
              </li>
            ))}
        ...
        {!loading && stats && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">{t("admin.dashboardPage.userDistribution")}</h2>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">{t("admin.dashboardPage.contentStats")}</h2>
              <Bar
                data={{
            ...
            {planDistribution.byUsers.length > 0 && (
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6">
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                  {t("admin.dashboardPage.usersByPlan")}
                </h2>
                <div className="flex justify-center">
                  <div style={{ maxWidth: "300px", width: "100%" }}>
                    <Doughnut
            ...
            {planDistribution.byRevenue.length > 0 && (
              <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6">
                <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                  {t("admin.dashboardPage.revenueByPlan")}
                </h2>
                <div className="flex justify-center">
                  <div style={{ maxWidth: "300px", width: "100%" }}>
                    <Doughnut
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6 flex flex-col">
            <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">{t("admin.dashboardPage.users")}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t("admin.dashboardPage.manageUsersDesc")}</p>
            <Link
              to="/admin/users"
            ...
            >
              {t("admin.dashboardPage.goToManagement")}
            </Link>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6 flex flex-col">
            <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">{t("admin.dashboardPage.upgradePlans")}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              {t("admin.dashboardPage.managePlansDesc")}
            </p>
            <Link
            ...
            >
              {t("admin.dashboardPage.manageUpgradePlans")}
            </Link>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6 flex flex-col">
            <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">{t("admin.dashboardPage.userPackages")}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              {t("admin.dashboardPage.manageUserPackagesDesc")}
            </p>
            <Link
            ...
            >
              {t("admin.dashboardPage.managePackages")}
            </Link>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-6 flex flex-col">
            <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">{t("admin.dashboardPage.commission")}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t("admin.dashboardPage.manageCommissionDesc")}</p>
            <Link
            ...
            >
              {t("admin.dashboardPage.manageCommission")}
            </Link>
          </div>
        </div>
      </div>
