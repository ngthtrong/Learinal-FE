...
function ExpertDashboardPage() {
  ...
  const loadData = async () => {
    try {
      ...
      console.error(e);
      setError(e?.response?.data?.message || t("expertPages.dashboard.errorLoadData"));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const cards = [
    {
      key: "earn_total",
      label: t("expertPages.dashboard.cards.totalEarned"),
    ...
    {
      key: "earn_pending",
      label: t("expertPages.dashboard.cards.pending"),
    ...
    {
      key: "validations",
      label: t("expertPages.dashboard.cards.pendingValidations"),
    ...
    {
      key: "avg_validation",
      label: t("expertPages.dashboard.cards.avgPerValidation"),
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-slate-900 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100">{t("expertPages.dashboard.pageTitle")}</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-xs sm:text-sm">{t("expertPages.dashboard.pageSubtitle")}</p>
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              ...
              disabled={refreshing}
            >
              {refreshing ? t("expertPages.common.refreshing") : t("expertPages.common.refresh")}
            </button>
          </div>
        </div>
        ...
        {(totalFixed > 0 || totalBonus > 0) && (
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-4 sm:p-6 mb-6 sm:mb-8">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-200 mb-3 sm:mb-4">{t("expertPages.dashboard.incomeBreakdown.title")}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div className="p-3 sm:p-4 rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border border-blue-200 dark:border-blue-700">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm sm:text-base font-medium text-blue-700 dark:text-blue-300">{t("expertPages.dashboard.incomeBreakdown.fixedRate")}</span>
                </div>
                <div className="text-xl sm:text-2xl font-bold text-blue-900 dark:text-blue-100">
                  {formatCurrency(totalFixed)}
                </div>
                <p className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 mt-1">
                  {t("expertPages.dashboard.incomeBreakdown.fixedRateDesc")}
                </p>
              ...
              <div className="p-3 sm:p-4 rounded-lg bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 border border-amber-200 dark:border-amber-700">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm sm:text-base font-medium text-amber-700 dark:text-amber-300">{t("expertPages.dashboard.incomeBreakdown.revenueBonus")}</span>
                </div>
                ...
                <p className="text-xs sm:text-sm text-amber-600 dark:text-amber-400 mt-1">
                  {t("expertPages.dashboard.incomeBreakdown.revenueBonusDesc")}
                </p>
                {hybridBreakdown.byType && (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-medium p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-200 mb-3 sm:mb-4">{t("expertPages.dashboard.quickGuide.title")}</h2>
          <ul className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-gray-600 dark:text-gray-400 list-disc pl-4 sm:pl-5">
            <li>{t("expertPages.dashboard.quickGuide.item1")}</li>
            <li>{t("expertPages.dashboard.quickGuide.item2")}</li>
            <li>{t("expertPages.dashboard.quickGuide.item3")}</li>
          </ul>
        </div>
