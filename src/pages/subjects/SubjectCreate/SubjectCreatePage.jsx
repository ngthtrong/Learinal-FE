...
const SubjectCreatePage = () => {
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      ...
      toast.showSuccess(t("subjects.createSuccess"));
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6">
        <div className="bg-white dark:bg-slate-800 shadow-sm border border-gray-200 dark:border-slate-700 rounded-lg px-4 sm:px-6 py-4 sm:py-6 mb-4 sm:mb-6">
          ...
          >
            ← <span className="hidden sm:inline">{t("subjects.backToList")}</span><span className="sm:hidden">{t("common.back")}</span>
          </button>
          <div className="space-y-2">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 dark:text-gray-100">{t("subjects.createPageTitle")}</h1>
            </div>
            <p className="text-sm sm:text-base lg:text-lg text-gray-600 dark:text-gray-400">
              {t("subjects.createPageSubtitle")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-6 sm:pb-8">
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 sm:p-6 lg:p-8">
          <SubjectForm
            ...
            loading={loading}
            submitText={t("subjects.createButton")}
          />
        </div>
      </div>
